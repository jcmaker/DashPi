...
3. 용도별 API 키를 만들고 키마다 사용 한도를 건다: `dashpi-pi` $2·**매일 초기화**, `dashpi-junhyeong` $5·초기화 없음(하네스 포함), 팀원별 $2·초기화 없음(약 35건). 키 한도에 걸리면 402(크레딧 부족 → 분석 실패)가 되므로, Pi 키의 하루 한도는 앱 하루 한도(20건 ≈ $1.12)보다 커야 한다 — 그래야 앱 한도가 먼저 걸려 '분석 대기 → 다음 날 자동 분석'이 된다.
