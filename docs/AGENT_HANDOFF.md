...
- ❌ **Raspberry Pi 네이티브 UI**: 디바이스 화면과 터치 입력
- ⚠️ **카메라 통합**: `rpicam-vid` 조각 녹화 코드와 합성 영상 테스트는 있으나 실제 Pi 검증 및 UI 연결은 아직 없음
...
| **사고 trigger** | 녹화 중 화면의 **사고 분석** 버튼 | `--trigger-seconds` CLI 인자 | 네이티브 화면 연결 필요 |
...
| **영상 소스** | Pi 카메라 실시간 녹화 | 샘플 MP4 파일, `rpicam-vid` 녹화 초안 | 실제 Pi 검증 필요 |
