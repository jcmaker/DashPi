        ...
        [
            ...
            "\n".join(
                [
                    ...
                    "receiver = TestClient(app).get('/receiver.html')",
                    "assert receiver.status_code == 200",
                    "assert '<title>DashPi 광학 수신</title>' in receiver.text",
                    "receiver_script = TestClient(app).get('/assets/receiver.js')",
                    "assert receiver_script.status_code == 200",
                    "assert len(receiver_script.content) > 1000",
                    "assert TestClient(app).get('/manifest.webmanifest').status_code == 200",
                    "assert TestClient(app).get('/icon.svg').status_code == 200",
                    "assert TestClient(app).get('/sw.js').status_code == 200",
                ]
            ),
        ],
        cwd=tmp_path,
